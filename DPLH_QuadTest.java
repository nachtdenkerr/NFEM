...
import plasticity.DruckerPrager_2D;
...
public class DPLH_QuadTest {
	public static void main (String args[]){
		...
		double E = 250.0, nuy = 0.3333, cohesion = 10.0;
		Quad quad = new Quad(E, nuy, "plane strain", 1.0, n1, n2, n3, n4);
		
		Force f = new Force (0.0, 10.0, 0.0);
		n3.setForce(f);
		n4.setForce(f);
		...
		double hardening = E/5.0;

		qstr.assignPlasticGaussPoints(4);

		for (GaussPoint gp : quad.getLOGP()){
			Material mat = new Material(1);
			gp.setMaterial(mat);
			DruckerPrager_2D vm = new DruckerPrager_2D(gp, cohesion, hardening, E, nuy, 0.0);
			gp.setPlasticModel(vm);
		}
		
		
		System.out.println("**********************************************************************");
		System.out.println("*********************  DRUCKER-PRAGER MATERIAL  **********************");
		System.out.println("**********************************************************************");
		System.out.println();
		
		NewtonRaphsonPla returnMapping = new NewtonRaphsonPla(qstr);
		returnMapping.solve(3, 0.0/2.0, 1.0/2.0);
